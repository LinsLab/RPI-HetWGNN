...
amino_acid = ['C','S','T','P','A', 'G', 'N','D','E','Q','H','R','K','M','I','L','V','F','Y','W']
...
def generated_SW_matrix(filename,path):
    ...
    pool = multiprocessing.Pool(processes=multiprocessing.cpu_count()) #多进程并发
    for i in range(len(allsequence)):
        savepath = path + str(i + 1) + '.txt'
        sequence1 = allsequence[i]
        sequence2 = allsequence[i:]
        pool.apply_async(s_w, (sequence1, sequence2, savepath, i,)) #返回一个子进程对象，非阻塞的，无序的
    pool.close() #将进程池关闭
    pool.join()  #阻塞等待所有子进程结束
    ...
    np.savetxt(os.path.join(path, r'protein_sw_mat.csv'), finalmatrix, delimiter=',', fmt='%f')

# p1="./data/dataset"+str(5)+"/"
# generated_SW_matrix(p1 + "protein_washed.fa", p1)
